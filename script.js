...
const CONFIG = {
  ...
  // Optional external direct audio file URL (mp3/etc — NOT a YouTube link).
  // Used only if localSong doesn't exist.
  songUrl: "",
...
el("back-btn").addEventListener("click", function () {
  // If the memory vault photo gallery is open on top of the current
  // screen, close that first instead of navigating chapters underneath it.
  const openGallery = qs(".gallery-viewer.is-active");
  if (openGallery) {
    openGallery.classList.remove("is-active");
...
(function buildLoveCards() {
  ...
  items.forEach(function (item) {
    ...
    card.innerHTML =
      ...
      '<span class="love-card-text">Tap to reveal ❤️</span>';

    let revealed = false;
    card.addEventListener("click", function () {
      if (revealed) return; // stays revealed once opened
      revealed = true;
      card.classList.add("is-open");
      qs(".love-card-text", card).textContent = item.text;
    });
...
/* ==========================================================================
   15. MUSIC PLAYER
   Tries, in order: local mp3 (audio/our-song.mp3) -> CONFIG.songUrl (direct
   audio file) -> CONFIG.youtubeId (hidden YouTube embed, audio only).
   ========================================================================== */
(function setupMusic() {
  ...
  function setPlayingUI(playing) {
    ...
    songBtn.textContent = playing ? "NOW PLAYING 🎵" : "PLAY OUR SONG 🎵";
  }

  /* ---- Path A: plain <audio> element (local file or direct URL) ---- */
  function tryAudioPlay(src) {
  ...
  /* ---- Path B: hidden YouTube embed, played via the IFrame API ---- */
        ...
        ytPlayer = new window.YT.Player(holder.id, {
          ...
          events: {
            ...
            onStateChange: function (e) {
              if (e.data === window.YT.PlayerState.PLAYING) setPlayingUI(true);
              if (e.data === window.YT.PlayerState.PAUSED || e.data === window.YT.PlayerState.ENDED) setPlayingUI(false);
            }
          }
        });
      });
  ...
  function playYouTube() {
    ensureYouTubePlayer().then(function (player) {
      mode = "youtube";
      player.playVideo();
    }).catch(function () {
      alert("Couldn't load the song right now — check your connection and try tapping again.");
    });
  }

  /* ---- Unified controls ---- */
  function play() {
    if (mode === "youtube" && ytPlayer) { ytPlayer.playVideo(); return; }
    if (mode === "audio") {
      audio.play().then(function () { setPlayingUI(true); }).catch(function () { fallbackChain(); });
      return;
    }
    fallbackChain();
  }
  ...
  function fallbackChain() {
    // 1) local file
    tryAudioPlay(CONFIG.localSong || "")
      .then(function () { setPlayingUI(true); })
      .catch(function () {
        // 2) direct external URL
        if (CONFIG.songUrl) {
          tryAudioPlay(CONFIG.songUrl)
            .then(function () { setPlayingUI(true); })
            .catch(function () { tryYouTubeOrFail(); });
        } else {
          tryYouTubeOrFail();
        }
      });
  ...
  function tryYouTubeOrFail() {
    if (CONFIG.youtubeId) {
      playYouTube();
    } else {
      alert("Add your song! Put an mp3 at audio/our-song.mp3, or set CONFIG.songUrl / CONFIG.youtubeId in script.js.");
    }
  }

  function pause() {
    if (mode === "youtube" && ytPlayer) { ytPlayer.pauseVideo(); return; }
    audio.pause();
    setPlayingUI(false);
  }
